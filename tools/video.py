...
def test_lanenet(image_path, weights_path, use_gpu, image_list, batch_size):
    ...
    with sess.as_default():
        ...
        while(True):
            ...
            instance_seg_image, existence_output = sess.run([binary_seg_ret, instance_seg_ret],feed_dict={input_tensor: image})
            time_end=time.time()
            print('inference time: ', time_end - time_start)
            for cnt_img in range(4):
                #cv2.imwrite(str(cnt_img + 1) + '_avg.png', (instance_seg_image[0, :, :, cnt_img + 1] * 255).astype(int) ) 
                ...
                if existence_output[0, cnt_img] > 0.5:
                    ...
                    for i in range(30):
                        if coordinate_tmp[0][i]>0:
                            cv2.circle(frame, (int(coordinate_tmp[0][i]*h/800), int(w-i*20)), 6, (0,0,255),-1)          
